...
Button Button_L(BUTTON_L_GPIO_Port, BUTTON_L_Pin, 2000);
Button Button_M(BUTTON_M_GPIO_Port, BUTTON_M_Pin, 2000);
Button Button_R(BUTTON_R_GPIO_Port, BUTTON_R_Pin, 2000);
...
/**
 * @brief  清除按键内置的计算值
 * @details  放置长时间delay后导致认为长时间再按着
 * @param
 */
void Button::clean(void)
{
	this->Last_State = Key_OFF;
	this->State = Key_OFF;
	this->OUT_State = Button_OFF;
	this->time_flag = MICROS_ms();
}
